...
public class GUIHelper
{
    ...
    {
        DrawSprite(sprite, width, height, Vector2.zero, drawBox);
    }
    ...
    public static void DrawSprite(Sprite sprite, float width, float height, Vector2 offset, bool drawBox = true)
    {
        ...
        if(sprite != null)
        {
            ...
            Rect r = new Rect(textureRect.x / texture.width, textureRect.y / texture.height, textureRect.width / texture.width, textureRect.height / texture.height);
...
public class Horizontal : IDisposable
{
    ...
    public Horizontal(GUIHelper.LayoutStyle style, params GUILayoutOption[] options)
    {
        EditorGUILayout.BeginHorizontal(style.ToString(), options);
    }
