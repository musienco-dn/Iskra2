...
{
    public class K2Saving : MonoBehaviour// ArrayPrefs2 v 1.4
    {
        #region Variables
		...
		fallback = false;
            ...
            if (Application.isEditor)
                savePath = Application.dataPath + "\\Save Files";
            else
		...
		fallback = true;
            ...
            //if we aren't going to use playerprefs, let's load the data
            if (!fallback)
                LoadSavedData();
        }
        ...
        static public int GetInt(string key)
        {
            ...
            for (int i = 0; i < intKeys.Count; i++) if (intKeys[i] == key)
                    return ints[i];
        ...
        static public float GetFloat(string key, float defaultVal)
        {
            ...
            for (int i = 0; i < floatKeys.Count; i++) if (floatKeys[i] == key)
                    return floats[i];
        ...
        static public string GetString(string key)
        {
            ...
            for (int i = 0; i < stringKeys.Count; i++) if (stringKeys[i] == key)
                    return strings[i];
        ...
        static public void Save()
        {
            ...
            //    outputString += intKeys[i] + "," + ints[i].ToString() + "\n";

            //outputString += "F\n";
            //for (int i = 0; i < floats.Count; i++)
            //    outputString += floatKeys[i] + "," + floats[i].ToString() + "\n";

            //outputString += "S\n";
            //for (int i = 0; i < strings.Count; i++)
            //    outputString += stringKeys[i] + "," + strings[i] + "\n";
        ...
        static void LoadSavedData()
        {
            ...
            string loadState = "";
            //bool gotKey = false; // This value is never used, so it's safe to leave commented out.
            ...
            for (int i = 0; i < loadedFile.Length; i++)
            {
                ...
                else
                {
                    //load a key
                    int endKeyIndex = -1;
                    for (int k = loadedFile[i].Length - 1; k >= 0; k--)
                    {
                        if (loadedFile[i].Substring(k, 1) == "," && endKeyIndex == -1)
                        {
                            endKeyIndex = k;
                        }
                    }

                    string loadKey = loadedFile[i].Substring(0, endKeyIndex);
                    string loadVar = loadedFile[i].Substring(endKeyIndex + 1, loadedFile[i].Length - endKeyIndex - 1);
        ...
        public static long GetLong(string key, long defaultValue)
        {
            ...
            lowBits = GetInt(key + "_lowBits");
            highBits = GetInt(key + "_highBits");
        ...
        {
            int lowBits = GetInt(key + "_lowBits");
            int highBits = GetInt(key + "_highBits");
